    ...
    pub fn output_value(
        ...
        let v = match &self.details {
            TypeEntryDetails::Enum(TypeEntryEnum {
                ...
                let name = if use_self { "Self" } else { name };
                match tag_type {
                    ...
                    EnumTagType::Adjacent { tag, content } => value_for_adjacent_enum(
                        type_space, name, variants, value, tag, content, scope,
                    )?,
            ...
            TypeEntryDetails::Newtype(TypeEntryNewtype { name, type_id, .. }) => {
                let inner = type_space
                    ...
                    .get(type_id)
                    .unwrap()
                    .output_value(type_space, value, scope, false);
                let ident = format_ident!("{}", name);
            ...
            TypeEntryDetails::Set(type_id) | TypeEntryDetails::Vec(type_id) => {
                ...
                let values = arr
                    .iter()
                    .map(|arr_value| inner.output_value(type_space, arr_value, scope, false))
                ...
                let kvs = obj
                    ...
                    .map(|(obj_key, obj_value)| {
                        Some((
                            ...
                            value_ty.output_value(type_space, obj_value, scope, false)?,
                        ))
                    })
                    .collect::<Option<Vec<_>>>()?;
    ...
    types
        ...
        .map(|(type_id, tup_value)| {
            type_space
                ...
                .get(type_id)
                .unwrap()
                .output_value(type_space, tup_value, scope, false)
        })
        .collect()
}
...
fn value_for_struct_props(
    ...
    let direct_props = map.iter().filter_map(|(name, value)| {
        ...
        let prop_value = type_entry.output_value(type_space, value, scope, false)?;
    ...
    let flat_props = properties.iter().filter_map(|prop| match &prop.rename {
        StructPropertyRename::Flatten => {
            ...
            let flat_value = type_entry.output_value(type_space, &extra_value, scope, false)?;
            let name = &prop.name;
...
mod tests {
    ...
    fn test_value_option() {
        ...
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!(null), &quote! {}, false)
                .map(|x| x.to_string()),
            Some("None".to_string()),
        );
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!(42), &quote! {}, false)
                .map(|x| x.to_string()),
    ...
    fn test_value_array() {
        ...
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!([]), &quote! {}, false)
                .map(|x| x.to_string()),
            Some("vec ! []".to_string()),
        );
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!([1, 2, 5]), &quote! {}, false)
                .map(|x| x.to_string()),
    ...
    fn test_value_map() {
        ...
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!({"a": 1, "b": 2}), &quote! {}, false)
                .map(|x| x.to_string()),
    ...
    fn test_value_tuple() {
        ...
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!([1, 2, "three"]), &quote! {}, false)
                .map(|x| x.to_string()),
    ...
    fn test_value_builtin() {
        ...
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!("not-a-uuid"), &quote! {}, false)
                .map(|x| x.to_string()),
            Some(
                quote! {
    ...
    fn test_value_bool() {
        ...
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!(true), &quote! {}, false)
                .map(|x| x.to_string()),
            Some("Some (true)".to_string()),
        );
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!(false), &quote! {}, false)
                .map(|x| x.to_string()),
    ...
    fn test_value_numbers_and_string() {
        ...
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!([0, 0, 0, "zero"]), &quote! {}, false)
                .map(|x| x.to_string()),
    ...
    fn test_enum_external_super_scoped() {
        ...
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!("A"), &quote! { super:: }, false)
                .map(|x| x.to_string()),
            Some(
                quote! {
                    super::Test::A
    ...
    fn test_enum_external() {
        ...
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!("A"), &quote! {}, false)
                .map(|x| x.to_string()),
            Some(
                quote! {
                    Test::A
    ...
    fn test_enum_untagged_super_scoped() {
        ...
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!(null), &quote! { super:: }, false)
                .map(|x| x.to_string()),
            Some(
                quote! {
                    super::Test::Variant0
    ...
    fn test_enum_untagged() {
        ...
        assert_eq!(
            type_entry
                .output_value(&type_space, &json!(["xx", "yy"]), &quote! {}, false)
                .map(|x| x.to_string()),
            Some(
                quote! {
