...
pub fn break_cycles(type_space: &mut TypeSpace, range: Range<u64>) {
    enum Node {
        Start {
            type_id: TypeId,
        },
    ...
    for id in range {
        ...
        // This isn't strictly necessary, but we'll short-circuit some work by
        ...
        while let Some(top) = stack.last_mut() {
            match top {
                ...
                Node::Start { type_id } => {
                    ...
                    // Determine which child types form cycles--and therefore
                    // need to be snipped--and the rest--into which we should
                    // descend. We make this its own block to clarify the
                    // lifetime of the exclusive reference to the type. We
                    // don't really *need* to have an exclusive reference here,
                    // but there's no point in writing `get_child_ids` again
                    // for shared references.
                    let (snip, descend) = {
                        let type_entry = type_space.id_to_entry.get_mut(type_id).unwrap();
                    ...
                    // TypeId will result in the same output TypeId. Ergo the
                    // resulting pairs from which we construct the mapping
                    // would contain exact duplicates; it would not contain two
                    // values associated with the same key.
                    let replace = snip
                        .into_iter()
                        .map(|type_id| {
                            let box_id = type_space.id_to_box(&type_id);
