...
impl TypeAndImpls {
    pub(crate) fn into_name_and_impls(self) -> (String, impl Iterator<Item = TypeSpaceImpl>) {
        // If there are no traits specified, these are assumed to be
        // implemented. A user would use the `?FromStr` syntax to remove one of
        // these defaults;
        const DEFAULT_IMPLS: [TypeSpaceImpl; 2] = [TypeSpaceImpl::FromStr, TypeSpaceImpl::Display];
