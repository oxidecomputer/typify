...
* The order of derives on types has stabilized (and therefore has changed) (#XXX)
* Specific `From` and `Deserialize` implementations for constrainted string types (#XXX)
* Specific `From` implementation for untagged enums with constrained string variants (#XXX)
* `FromStr` implementation for simple-variant-only `enum`s (#XXX)
* Ignore unknown `format` values (#XXX)
* Added `regress` dependency for ECMA 262 style regexes (#XXX)
